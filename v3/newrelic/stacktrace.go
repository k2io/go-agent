...
func (f stacktraceFrame) formattedName() string {
	if strings.HasPrefix(f.Name, "go.") {
		...
		// "go.(*struct { github.com/k2io/go-agent.threadWithExtras }).NoticeError"
		return f.Name
	}
...
func (f stacktraceFrame) isAgent() bool {
	...
	// "go.(*struct { github.com/k2io/go-agent.threadWithExtras }).NoticeError"
	return strings.Contains(f.Name, "github.com/k2io/go-agent/v3/internal.") ||
		strings.Contains(f.Name, "github.com/k2io/go-agent/v3/newrelic.")
}
